...
#fi
# Local Slave IP
export SPARK_SLAVE_IP="$(hostname -i)"
# Master IP
#export IP=$1
export IP="10.1.2.17"
export PRT="7077"
...
if ! [[ IP == "" ]]; then
	echo -e "Launching Slave Node - - -\n\tDetermined Master: ${IP}\n"
else
	echo -e "Unable to Launch Slave Node - - -\n\tCould Not Determine Master IP.\n"
fi
...
adduser smack -D /home/smack
mkdir /home/smack/.ssh
# Setup Private Key
cat << EOF > /home/smack/.ssh/rsa_id
...
EOF
# Setup Public Key
cat << EOF > /home/smack/.ssh/rsa_id.pub
...
EOF
# Add to Known Hosts
cat << EOF >> /home/smack/.ssh/authorized_keys
ssh-rsa  smack@spark-master
EOF
# START MASTER SERVICE
$SPARK_DIR/sbin/start-slave.sh spark://${IP}:${PRT}
# Create Bashrc File
cat << EOF >> /etc/bashrc
# Added by SMACK
#---------------
